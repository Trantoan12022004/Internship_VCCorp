...
import { analyzeQuery } from "../sevices/api";
import { marked } from "marked";
import ChartView from "./ChartView"; // Thêm dòng này ở đầu file
...
export default class Services extends Component {
    ...
    sendMessage = async (e) => {
        ...
        try {
            ...
            const selectedFunction = response.data.selectedFunction;

            const systemMessage = {
                id: Date.now() + 1,
                text: response.data.message || "Đã xử lý xong yêu cầu.",
                sender: "system",
                timestamp: new Date().toLocaleTimeString(),
            };
            ...
            this.setState((prevState) => ({
                ...
                isLoading: false,
                chartData: Array.isArray(apiData) && apiData.length > 0 ? apiData : null,
    ...
    render() {
        const { messages, inputMessage, isLoading, chartData, selectedFunction } = this.state;
                            ...
                            <Col lg={8}>
                                <div className="card chat-container p-4">
                                    ...
                                    <div className="d-flex justify-content-between align-items-center mb-4">
                                        ...
                                        <button className="btn btn-sm btn-outline-secondary" onClick={this.clearChat}>
                                            Xóa chat
                                        </button>
                                    </div>
                                    ...
                                    >
                                        {messages.map((message) => (
                                            <div key={message.id} className={`message mb-3 d-flex ${message.sender === "user" ? "justify-content-end" : "justify-content-start"}`}>
                                                <div
                                                    className={`message-content p-3 rounded ${message.sender === "user" ? "bg-primary text-white" : "bg-white border"}`}
                                                    style={{
                                                        maxWidth: "70%",
                                                        wordWrap: "break-word",
                                                    }}
                                    ...
                                    <form onSubmit={this.sendMessage} className="d-flex gap-2">
                                        ...
                                        />
                                        <button type="submit" disabled={!inputMessage.trim() || isLoading} className="btn btn-primary" style={{ minWidth: "80px" }}>
                                            {isLoading ? "Đang gửi..." : "Gửi"}
                                        </button>
                                    </form>

                                    {/* Chart View */}
                                    {this.state.chartData && this.state.selectedFunction && (
                                        <ChartView chartData={this.state.chartData} selectedFunction={this.state.selectedFunction} />
                                    )}
                                </div>
                            </Col>
                        </Row>
