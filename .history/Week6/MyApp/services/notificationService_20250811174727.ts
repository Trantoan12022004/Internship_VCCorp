...
class NotificationService {
  ...
  async scheduleTaskNotification(task: Task): Promise<string | null> {
    try {
      ...
      const scheduleTime = notificationTime <= new Date() ? new Date(Date.now() + 5000) : notificationTime;
      ...
      console.log(`Scheduled notification for task ${task._id} at ${scheduleTime.toLocaleString()}`);
