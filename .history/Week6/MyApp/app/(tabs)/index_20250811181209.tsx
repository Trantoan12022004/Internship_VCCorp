...
import { getTask, updateTask, createTask, deleteTask, loadCalendar } from "../../services/api";
import AddTaskModal from "../../components/AddTaskModal";
import NotificationService from "../../services/notificationServi";
...
export default function HomeScreen() {
            ...
            if (hasPermission) {
                ...
                const responseListener = Notifications.addNotificationResponseReceivedListener(
                    (response) => {
                        ...
                        // Có thể navigate đến task detail hoặc làm gì đó khi user tap notification
                        const taskId = response.notification.request.content.data?.taskId;
                        if (taskId) {
                            console.log("User tapped notification for task:", taskId);
                            // TODO: Navigate to task detail or perform some action
                        }
                    }
                );
                ...
                Alert.alert(
                    "Thông báo",
                    "Ứng dụng cần quyền gửi thông báo để nhắc nhở bạn về các tasks. Vui lòng cấp quyền trong Settings.",
                    [{ text: "OK" }]
                );
            }
        } catch (error) {
    ...
    // Tạo calendar days cho tuần hiện tại
    ...
    useFocusEffect(
        useCallback(() => {
            fetchTasks();
        }, [fetchTasks]) // Thêm fetchTasks vào dependency
    );

    // Tạo calendar days khi component mount
    useEffect(() => {
        generateCalendarDays();
    }, [generateCalendarDays]);

    // ========== HELPER FUNCTIONS ==========
    // Tính toán thống kê tasks cho tuần hiện tại
    ...
    // Fetch tasks từ API và lên lịch notifications
    const fetchTasks = useCallback(async () => {
        try {
            ...
            if (response.data.success) {
                ...
                // Lên lịch notifications cho tasks
                await notificationService.scheduleTasksNotifications(allTasks);
    ...
    }, [calculateStats, notificationService]);
    ...
    // Toggle task completion với notification handling
    const toggleTaskCompletion = async (taskId: string) => {
        ...
        try {
            ...
            if (response.data && response.data.success) {
                ...
                // Nếu task được hoàn thành, hủy notification
                if (newCompletionStatus) {
                    await notificationService.handleTaskCompleted(taskId);
                }

                // Cập nhật lại task với data từ server
                ...
                // Lên lịch lại notifications sau khi cập nhật
                await notificationService.scheduleTasksNotifications(finalUpdatedTasks);
            } else {
            ...
            },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        if (response.data.success) {
                            // Hủy notification của task bị xóa
                            await notificationService.handleTaskCompleted(taskId);
                            ...
                            // Lên lịch lại notifications
                            await notificationService.scheduleTasksNotifications(updatedTasks);
