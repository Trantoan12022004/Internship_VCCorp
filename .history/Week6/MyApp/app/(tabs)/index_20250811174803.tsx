...
export default function HomeScreen() {
    ...
    const initializeNotifications = async () => {
        try {
            ...
            // Lắng nghe notifications khi app đang mở
    ...
    const handleNotificationTap = (taskId: string) => {
        // Tìm task và focus vào ngày của task đó
        const task = tasks.find(t => t._id === taskId);
        if (task) {
            const taskDate = new Date(task.date);
            setSelectedDate(taskDate);
            
            // Cập nhật calendar days để highlight ngày được chọn
            const updatedDays = calendarDays.map((day) => ({
                ...day,
                isActive: day.fullDate.toDateString() === taskDate.toDateString(),
            }));
            setCalendarDays(updatedDays);
            
            Alert.alert('Task Reminder', `Don't forget: ${task.title}`);
        }
    };
    ...
    const fetchTasks = useCallback(async () => {
        try {
            ...
            if (response.data.success) {
                ...
                // Schedule notifications cho tất cả tasks
                await notificationService.scheduleMultipleTaskNotifications(allTasks);
            } else {
    ...
    const toggleTaskCompletion = async (taskId: string) => {
        ...
        try {
            ...
            if (response.data && response.data.success) {
                ...
                // Cancel notification nếu task được complete
                if (newCompletionStatus) {
                    await notificationService.cancelTaskNotification(taskId);
                } else {
                    // Schedule lại notification nếu task được uncomplete
                    await notificationService.scheduleTaskNotification(serverUpdatedTask);
                }
            } else {
                ...
                onPress: async () => {
                    try {
                        ...
                        if (response.data.success) {
                            ...
                            // Cancel notification cho task đã xóa
                            await notificationService.cancelTaskNotification(taskId);
    ...
    return (
        <SafeAreaView style={styles.container}>
            {/* Header - thêm notification test button */}
            <View style={styles.header}>
                <TouchableOpacity style={styles.headerIcon} onPress={testNotification}>
