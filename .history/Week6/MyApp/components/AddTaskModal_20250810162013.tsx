...
export default function AddTaskModal({
    ...
    const formatDateForAPI = (date: Date): string => {
        // Tạo date object với local timezone, set time về 00:00:00
        ...
        console.log("Local date for API:", localDate);
        console.log("ISO string:", localDate.toISOString());

        return localDate.toISOString();
    };

    // Hoặc sử dụng cách này để format date không bị lệch timezone:
    const formatDateForAPI_Alternative = (date: Date): string => {
        const year = date.getFullYear();
        const month = String(date.getMonth() + 1).padStart(2, "0");
        const day = String(date.getDate()).padStart(2, "0");

        // Trả về format ISO-like nhưng không có timezone offset
        const dateString = `${year}-${month}-${day}T00:00:00.000Z`;

        console.log("Date for API:", dateString);
        return dateString;
    };
