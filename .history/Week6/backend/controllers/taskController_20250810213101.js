        ...
        const job = cron.schedule(cronPattern, async () => {
            try {
                ...
                if (!currentTask || currentTask.is_completed) {
                    console.log(`Task ${task._id} đã hoàn thành hoặc không tồn tại, bỏ qua notification`);
                    return;
                }
                ...
                const notificationData = {
                    ...
                    message: `Task "${task.title}" sẽ bắt đầu trong 15 phút (${taskDateTime.format('HH:mm')})`,
                    type: 'TASK_REMINDER',
    ...
    try {
        ...
        if (updateFields.hasOwnProperty('is_completed')) {
            if (updateFields.is_completed === true) {
                ...
                console.log(`Task ${req.params.id} được đánh dấu chưa hoàn thành, đã schedule lại notification`);
            }
            ...
            if (!updatedTask.is_completed) {
                cancelTaskNotification(req.params.id);
                scheduleTaskNotification(updatedTask);
                console.log(`Task ${req.params.id} thay đổi thời gian, đã schedule lại notification`);
            }
        }
