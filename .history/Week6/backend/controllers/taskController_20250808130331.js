...
const getTasksGoogleCalendar = async (req, res) => {
    try {
        ...
        // Thiết lập thời gian mặc định (7 ngày tới)
        ...
        const tasks = events.map(event => {
            ...
            return {
                ...
                time: startTime && event.start?.dateTime ? 
                      new Date(startTime).toTimeString().slice(0, 5) : null,
                startTime: startTime,
                endTime: endTime,
                status: event.status,
        ...
        console.log(`Successfully fetched ${tasks.length} events from Google Calendar`);
