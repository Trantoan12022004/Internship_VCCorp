...
// Map để lưu các scheduled jobs
const scheduledJobs = new Map();

// Hàm schedule notification cho task
const scheduleTaskNotification = (task) => {
    try {
        // Chỉ schedule cho task chưa hoàn thành
        if (task.is_completed) {
            console.log(`Task ${task._id} đã hoàn thành, không schedule notification`);
            return;
        }

        // Combine date và time thành datetime
        const taskDateTime = moment.tz(
            `${task.date.toISOString().split("T")[0]} ${task.time}`,
            "YYYY-MM-DD HH:mm",
            "Asia/Ho_Chi_Minh"
        );

        // Tính thời gian notification (trước 15 phút)
        const notificationTime = taskDateTime.clone().subtract(15, "minutes");

        // Kiểm tra xem thời gian notification có hợp lệ không (phải trong tương lai)
        if (notificationTime.isBefore(moment())) {
            console.log(`Task ${task._id} - Thời gian notification đã qua, không schedule`);
            return;
        }
        ...
        // Tạo cron pattern từ notification time
        const cronPattern = `${notificationTime.second()} ${notificationTime.minute()} ${notificationTime.hour()} ${notificationTime.date()} ${
        ...
        const job = cron.schedule(
            ...
            async () => {
                try {
                    ...
                    // Kiểm tra lại trạng thái task trong DB
                    const currentTask = await Task.findById(task._id);
                    if (!currentTask || currentTask.is_completed) {
                        console.log(
                            `Task ${task._id} đã hoàn thành hoặc không tồn tại, bỏ qua notification`
                        );
                        return;
                    }

                    // Gửi notification (có thể mở rộng để gửi push notification, email, etc.)
                    const notificationData = {
                        ...
                        message: `Task "${
                            task.title
                        }" sẽ bắt đầu trong 15 phút (${taskDateTime.format("HH:mm")})`,
                        type: "TASK_REMINDER",
                        timestamp: new Date().toISOString(),
                    };

                    // Log notification (sau này có thể save vào DB hoặc gửi push notification)
                    console.log("📱 NOTIFICATION DATA:", notificationData);

                    // Có thể emit socket event cho frontend real-time
                    // if (io) {
                    //     io.emit('task_reminder', notificationData);
                    // }

                    // Cleanup job sau khi chạy
                    scheduledJobs.delete(task._id.toString());
                } catch (error) {
        ...
        // Lưu job để có thể hủy sau này
        scheduledJobs.set(task._id.toString(), job);

        console.log(`✅ Đã schedule notification cho task ${task._id}`);
    } catch (error) {
        console.error("Lỗi khi schedule notification:", error);
    }
};

// Hàm hủy notification khi task hoàn thành
const cancelTaskNotification = (taskId) => {
    const jobKey = taskId.toString();
    if (scheduledJobs.has(jobKey)) {
        scheduledJobs.get(jobKey).destroy();
        scheduledJobs.delete(jobKey);
        console.log(`🗑️ Đã hủy notification cho task ${taskId}`);
    }
};

// Hàm khởi tạo lại notifications khi server restart
const initializeTaskNotifications = async () => {
    try {
        console.log("🔄 Initializing task notifications...");
...
const createTask = async (req, res) => {
    try {
        ...
        // Schedule notification cho task mới
        scheduleTaskNotification(newTask);

        res.status(201).json({
            success: true,
            message: "Task được tạo thành công và đã schedule notification",
            data: newTask,
        });
    } catch (error) {
...
const updateTask = async (req, res) => {
    try {
        ...
        // Xử lý notification dựa trên thay đổi
        if (updateFields.hasOwnProperty("is_completed")) {
            if (updateFields.is_completed === true) {
                // Task được đánh dấu hoàn thành, hủy notification
                cancelTaskNotification(req.params.id);
                console.log(`Task ${req.params.id} được đánh dấu hoàn thành, đã hủy notification`);
            } else if (updateFields.is_completed === false) {
                // Task được đánh dấu chưa hoàn thành, schedule lại notification
                scheduleTaskNotification(updatedTask);
                console.log(
                    `Task ${req.params.id} được đánh dấu chưa hoàn thành, đã schedule lại notification`
                );
            }
        } else if (updateFields.date || updateFields.time) {
            // Nếu thay đổi thời gian task và task chưa hoàn thành, schedule lại notification
            if (!updatedTask.is_completed) {
                cancelTaskNotification(req.params.id);
                scheduleTaskNotification(updatedTask);
                console.log(
                    `Task ${req.params.id} thay đổi thời gian, đã schedule lại notification`
                );
            }
        }
...
const toggleTaskComplete = async (req, res) => {
    try {
        ...
        // Xử lý notification dựa trên trạng thái mới
        if (updatedTask.is_completed) {
            // Task được đánh dấu hoàn thành, hủy notification
            cancelTaskNotification(req.params.id);
            console.log(`Task ${req.params.id} được đánh dấu hoàn thành, đã hủy notification`);
...
const suggestAI = async (req, res) => {
    try {
        ...
        if (!title || !date) {
            return res.status(400).json({
                success: false,
                message: "Vui lòng điền đầy đủ thông tin: title, date",
            });
        }
...
Trả lời bằng JSON format chính xác:
{
...
CHỈ trả về JSON, không có text thêm.`;
        ...
        try {
            ...
            return res.status(500).json({
                success: false,
                message: "Lỗi parse response từ AI",
        ...
        if (!aiSuggestion.recommendedTime || !aiSuggestion.reason) {
            return res.status(500).json({
                success: false,
                message: "AI response không có đủ thông tin cần thiết",
                rawResponse: text,
            });
        }
        ...
        if (!timeRegex.test(aiSuggestion.recommendedTime)) {
            console.warn("⚠️ AI returned invalid time format:", aiSuggestion.recommendedTime);
            // Có thể set default time hoặc return error
            aiSuggestion.recommendedTime = "09:00"; // fallback time
        }

        console.log("✅ AI Suggestion processed successfully:", aiSuggestion);

        res.status(200).json({
            success: true,
            message: "Gợi ý thời gian từ AI thành công",
            data: {
                taskInfo: {
                    title,
        ...
        console.error("❌ Error in suggestAI:", error.message);
        console.error("❌ Error stack:", error.stack);

        res.status(500).json({
            success: false,
            message: "Lỗi khi gợi ý thời gian từ AI",
            error: error.message,
        });
    }
};

// @desc    Lấy thông tin về scheduled notifications
// @route   GET /api/task/notifications
// @access  Public
const getScheduledNotifications = async (req, res) => {
    try {
        console.log("=== GET SCHEDULED NOTIFICATIONS ===");
            ...
            try {
                ...
                if (task && !task.is_completed) {
                    ...
                    notifications.push({
                        taskId: task._id,
                        title: task.title,
                        taskTime: taskDateTime.format("YYYY-MM-DD HH:mm"),
                        notificationTime: notificationTime.format("YYYY-MM-DD HH:mm"),
                        isActive: !notificationTime.isBefore(moment()),
                        timeToNotification: notificationTime.fromNow(),
                    });
                }
            } catch (error) {
        ...
        res.status(500).json({
            success: false,
            message: "Lỗi khi lấy thông tin notifications",
            error: error.message,
        });
    }
};
