...
const suggestAI = async (req, res) => {
    try {
        ...
        if (!title || !date) {
            return res.status(400).json({
                success: false,
                message: "Vui lòng điền đầy đủ tiêu đề và ngày",
            });
        }
...
`;
        ...
        let suggestion;
        try {
            // Tìm và parse JSON từ response text
            ...
            // Fallback suggestion
            suggestion = getFallbackSuggestion(title, description, taskDate);
        }

        // Validate thời gian
        const timeRegex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/;
        if (!timeRegex.test(suggestion.recommendedTime)) {
            suggestion.recommendedTime = "14:00";
        }
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                suggestion: {
                    ...
                    reason: suggestion.reason || "Thời gian phù hợp cho loại công việc này",
                    confidence: suggestion.confidence || 0.8,
                    workType: suggestion.workType || "Công việc chung",
                    tips: suggestion.tips || "Nên chuẩn bị trước 15 phút"
                },
                rawResponse: text // Để debug
            }
        });

    } catch (error) {
        console.error("=== ERROR IN AI SUGGESTION ===");
        console.error("Error message:", error.message);
        console.error("Error stack:", error.stack);
        ...
        if (error.message.includes("API_KEY")) {
            return res.status(401).json({
                ...
                error: "Vui lòng kiểm tra GEMINI_API_KEY trong .env"
            });
        }
        ...
        if (error.message.includes("quota") || error.message.includes("limit")) {
            return res.status(429).json({
                success: false,
                message: "Đã vượt quá giới hạn API",
        ...
        // Fallback khi có lỗi
        try {
            const fallbackSuggestion = getFallbackSuggestion(req.body.title, req.body.description, new Date(req.body.date));
            
            return res.status(200).json({
                success: true,
                message: "Gợi ý thời gian (fallback)",
                data: {
                    task: {
                        title: req.body.title,
                        description: req.body.description,
                        date: new Date(req.body.date).toLocaleDateString('vi-VN')
                    },
                    suggestion: fallbackSuggestion,
                    note: "Sử dụng logic cơ bản do AI tạm thời không khả dụng"
                }
            });
        } catch (fallbackError) {
            return res.status(500).json({
                success: false,
                message: "Lỗi khi gợi ý thời gian",
                error: error.message,
            });
        }
    }
...
// Hàm fallback logic
const getFallbackSuggestion = (title, description, date) => {
    const text = (title + " " + (description || "")).toLowerCase();
    const dayOfWeek = date.getDay();
    const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
    
    let recommendedTime = "14:00";
    let workType = "Công việc chung";
    let reason = "Thời gian cân bằng trong ngày";
    let tips = "Nên chuẩn bị trước 15 phút";
    
    // Phân tích loại công việc
    if (text.includes("meeting") || text.includes("họp") || text.includes("gặp")) {
        recommendedTime = isWeekend ? "10:00" : "09:30";
        workType = "Cuộc họp";
        reason = "Buổi sáng giúp mọi người tập trung và sẵn sàng tham gia";
        tips = "Chuẩn bị agenda trước 30 phút";
    } else if (text.includes("email") || text.includes("mail") || text.includes("thư")) {
        recommendedTime = isWeekend ? "11:00" : "08:30";
        workType = "Xử lý email";
        reason = "Buổi sáng sớm là thời điểm tốt nhất để xử lý email";
        tips = "Nên xử lý email quan trọng trước";
    } else if (text.includes("code") || text.includes("lập trình") || text.includes("coding")) {
        recommendedTime = isWeekend ? "14:00" : "10:00";
        workType = "Lập trình";
        reason = "Thời điểm này não bộ hoạt động tốt nhất cho việc coding";
        tips = "Tắt thông báo để tập trung tối đa";
    }
