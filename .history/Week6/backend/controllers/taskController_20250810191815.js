...
CHỈ trả về JSON, không có text thêm.`;
        ...
        try {
            ...
            aiSuggestion = {
                ...
                reason: "Không thể phân tích gợi ý từ AI. Đây là thời gian mặc định phù hợp để bắt đầu công việc.",
                confidence: 0.5,
                workType: "Không xác định",
                tips: "Hãy thử lại hoặc liên hệ hỗ trợ.",
            };
        }

        // Validate AI response structure
        const requiredFields = ["recommendedTime", "reason", "confidence", "workType", "tips"];
        const missingFields = requiredFields.filter((field) => !aiSuggestion.hasOwnProperty(field));
        ...
        if (!timeRegex.test(aiSuggestion.recommendedTime)) {
            console.warn("Invalid time format from AI:", aiSuggestion.recommendedTime);
        ...
        if (error.message.includes("API key")) {
            return res.status(401).json({
                success: false,
                message: "Lỗi xác thực API key Gemini",
