...
const getTasksGoogleCalendar = async (req, res) => {
    try {
        ...
        // Thiết lập authentication với Service Account
        const auth = new google.auth.GoogleAuth({
            keyFile: path.join(__dirname, '../config/google-calendar-service-account.json'), // Đường dẫn đến file service account
        ...
        // Thiết lập thời gian mặc định (tuần này)
        const now = new Date();
        const startOfWeek = new Date(now);
        ...
        // Chuyển đổi events thành định dạng task
        const tasks = events.map(event => {
            ...
            return {
                ...
                time: event.start.dateTime ? 
                    new Date(startDateTime).toLocaleTimeString('vi-VN', { 
                        hour: '2-digit', 
                        minute: '2-digit',
                        hour12: false 
                    }) : '00:00',
                endTime: event.end.dateTime ? 
                    new Date(endDateTime).toLocaleTimeString('vi-VN', { 
                        hour: '2-digit', 
                        minute: '2-digit',
                        hour12: false 
                    }) : '23:59',
                is_completed: false, // Google Calendar events không có trạng thái completed
                location: event.location || '',
                attendees: event.attendees || [],
                htmlLink: event.htmlLink,
                source: 'google_calendar',
                isAllDay: !event.start.dateTime, // Nếu không có dateTime thì là sự kiện cả ngày
        ...
        if (error.code === 401) {
            return res.status(401).json({
                success: false,
                message: "Không có quyền truy cập Google Calendar. Vui lòng kiểm tra authentication.",
                error: error.message,
            });
        }
        ...
        if (error.code === 403) {
            return res.status(403).json({
                success: false,
                message: "API quota exceeded hoặc không có quyền truy cập.",
                error: error.message,
            });
        }
        ...
        if (error.code === 404) {
            return res.status(404).json({
                success: false,
                message: "Không tìm thấy calendar với ID đã chỉ định.",
                error: error.message,
            });
        }
